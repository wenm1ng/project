...
class ShopUserModel extends Model
{
     ...
     * @return bool
     */
    public function checkShopUserName($username)
    {
        $table_name = 'dineshop_info';
        $ret = Db::name($table_name)->field('f_sid as sid')->where('f_username', $username)->find();
        if (empty($ret)) {
            return false;
        }
        return $ret;
    ...
    public function setCkExpired($ck){
        $table_name = 'admin_login';
        $data = array(
            'f_expiretime' => date("Y-m-d H:i:s", time()-3000),
        );
        $retup = Db::name($table_name)
            ->where('f_usercheck',$ck)
            ->update($data);
    ...
    public function addUserLogin($ck, $uid, $ip){
        ...
        $userinfo1 = Db::name($table_name)
            ->where('f_usercheck',$ck)
            ->find();
        if($userinfo1){
            $newtime1 = date("Y-m-d H:i:s", time()+1800);
            $newtime2 = date('Y-m-d H:i:s');
            $result = Db::name($table_name)
            ->where('f_usercheck',$ck)
            ->update(['f_expiretime' => "$newtime1",'f_lasttime' => "$newtime2"]);
            if($result){
                return array(
                    'ck' => $ck,
                    'uid' => $uid,
     ...
     * @return array|bool
     */
    public function addShopLogin($ck, $sid, $ip){
        $table_name = 'dineshop_login';
        //判断用户是否重复登录
        $userinfo = Db::name($table_name)
            ->where('f_sid',$sid)
            ->where('f_expiretime', '> time', time())
     ...
     */
    public function getLoginShopInfo($ck,$uid){
        $table_name = 'dineshop_login';
        $userinfo = Db::name($table_name)
            ->where('f_sid',$uid)
            ->where('f_usercheck',$ck)
     ...
     * @return array|false|\PDOStatement|string|Model
     */
    public function getShopInfoByUid($sid){
        $table_name = 'dineshop_info';
        $userinfo = Db::name($table_name)
            ->where('f_sid',$sid)
            ->field('f_sid as sid')
     ...
     * @return bool
     */
    public function checkShopUserStatus($sid){
        $userinfo = self::getShopInfoByUid($sid);
