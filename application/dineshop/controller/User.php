...
use base\Baseshop;
use \app\dineshop\model\UserModel;
...
class User extends Baseshop
{
    ...
    public function getRegUserList(){
        $info = array();
        $list = array();//http://localhost/wenming/Service-master/
        // $page = input('page',1); //页码
        // $pagesize = input('pagesize',20); //每页显示数
        $model = new UserModel();
        // $res = $model->getUserList($page, $pagesize);
        $res = $model->getUserList();
        $status = array(100=>'禁用',-100=>'关闭禁用');
        $sta = array(100=>'启用',-100=>'禁用');
        // $info['allnum'] = $res['allnum'];
        // $list = $res['userlist'];
        // $list = $res['userlist'];
        // return json($this->sucjson($info, $list));
        // var_dump($res);
        return view('users/index',['list'=>$res,'status'=>$status,'sta'=>$sta]);
    }
    ...
    {   
        // if(session::get('uid') && session::get('ck')){
        //     return json(self::sucres("登录成功")); 
        // }
        ...
        // return json(self::erres($userinfo['uid']));
        if(empty($userinfo)){
            return $this->error('用户ID不存在');
            // return json(self::erres("用户ID不存在"));
        }
        // return json(array(strtoupper(md5($password)),$userinfo['password']));
        ...
        //写登录信息
        $ck = 'ck_' . strtoupper(base64_encode(md5($userinfo['uid'].'wenminghenshuai')));
        $ret_login = $this->model->addUserLogin($ck,$this->uid,$ip);
        if ($ret_login === false) {
            return $this->error('写登录信息失败');
            // return json(self::erres("写登录信息失败"));
        }
        session::set('ck',$ret_login['ck']);
        session::set('uid',$ret_login['uid']);
        $resinfo = array(
            'ck' => $ret_login['ck'],
            'uid' => $ret_login['uid'],
        );
    ...
    public function logindineshop()
    {
        ...
        $ret_user = $this->model->checkShopUserName($username);
        if(false === $ret_user){
            // return json(self::erres("用户名不存在"));
            return $this->error('用户名不存在');
            // $this->error('用户名不存在');
        }
        $this->sid = $ret_user['sid'];

        $userinfo = $this->model->getShopInfoByUid($this->sid);
        // return json(self::erres($userinfo['uid']));
        ...
        if(!$this->model->checkShopUserStatus($this->sid)){
            return $this->error('该用户已被禁用');
            // return json(self::erres("该用户已被禁用"));
        }

        //写登录信息
        $ck = 'ck_' . strtoupper(base64_encode(md5($userinfo['sid'].'jiegehenshuai')));
        $ret_login = $this->model->addShopLogin($ck,$this->sid,$ip);
        if ($ret_login === false) {
            return $this->error('写登录信息失败');
            // return json(self::erres("写登录信息失败"));
        }
        session::set('ck',$ret_login['ck']);
        session::set('sid',$ret_login['sid']);
        $resinfo = array(
            'ck' => $ret_login['ck'],
            'sid' => $ret_login['sid'],
        );
        // var_dump($userinfo);exit;
            return $this->success('登录成功',url('dineshop/index/index'));
        // return json(self::sucres("登录成功"));
    }
    ...
    public function logout()
    {
        ...
        if ($this->model->setCkExpired($this->ck)) {
            // return json(self::sucres());
            return $this->success('退出登录成功!',url('admin/Index/login'));
        } else {
     ...
     * @time 2017/7/26
     */
    public function openUserStatus()
    {
        $userid = $_POST['userid'];
        $status = -$_POST['status'];
        $user = new UserModel();
        $suc = $user->updateUserInfo($userid,$status);
