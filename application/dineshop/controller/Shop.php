...
class Shop extends Base
{
    ...
    public function getDineshopSell(){
        ...
        // print_r($list);
        // return json($this->sucjson($info, $list));
        return view('getdineshopsell',['list'=>$list,'shopid'=>$shopid,'desklist'=>$desklist]);
    }
    /**
     * 添加店铺放号信息
     */
    public function addDineshopSell(){
        $timeid = Db::table('t_dineshop_deskinfo')->field("f_deskid")->select();
        foreach ($timeid as $key => $val) {
            $deskinfo = $_POST['sellinfo'.$val['f_deskid']];
            $deskinfo = $val['f_deskid'].'@'.$deskinfo;
            // print_r(input());exit;
            $info = array();
            $list = array();
    ...
    public function getDesklist(){
        ...
        // print_r($list);
        // return json($this->sucjson($info, $list));
        return view('getdesklist',['list'=>$list,'shopid'=>$shopid]);
    }
    /**
     * 获取店铺桌型信息
     */
