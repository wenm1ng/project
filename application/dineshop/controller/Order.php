...
use base\Baseshop;
use \think\Session;
use \app\dineshop\model\OrderModel;
use \app\dineshop\model\DishesModel;
use \app\dineshop\model\TastesModel;
...
class Order extends Baseshop
{
    ...
    public function getOrderlist(){
        ...
        // echo 11;
        $orderslist = $orders->getAllOrderList($this->sid);
        // echo "<pre>";
    ...
    public function getOneOrder()
    {
        ...
        $orders = new OrderModel();
        $orderslist = $orders->findOne($name,$dataDay,$dataDay1,$timeDay,$timeDay1,$this->sid);
            // echo json_encode($orderslist);
        // $orderslist[0]['id'] = 1;
    ...
    public function getFood()
    {
        $this->sid = session::get('sid');
        $orders = new OrderModel();
        $foodlist = $orders->getAllAdvance($this->sid);
    ...
    public function searchAdvance()
    {
        ...
        $orders = new OrderModel();
        $foodlist = $orders->getSearchAdvance($foodname,$this->sid);
        if(!empty($foodlist)){
    ...
    public function getSearchTime()
    {
        ...
        $foodlist = $order->getSearchTimer($dataDay,$dataDay1,$timeDay,$timeDay1,$foodname,$this->sid);
        echo json_encode($foodlist);
    }
    ...
    public function getTakeOut()
    {
        $this->sid = session::get('sid');
        $order = new OrderModel();
        $list = $order->getAllTakeOut($this->sid);
        // echo "<pre>";
        // var_dump($list);exit;
    ...
    public function editStatus()
    {
        ...
        $oid = $_POST['oid'];
        $orders = new OrderModel();
        $list = $orders->editStatus($this->sid,$oid,$status);
    ...
    public function todayList()
    {
        $this->sid = session::get('sid');
        $orders = new OrderModel();
        $list = $orders->getTodayList($this->sid);
        return view("orders/todaylist");
    }
    // /**
    //  * 订单处理
    //  */
