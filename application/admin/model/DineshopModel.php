...
class DineshopModel extends Model
{
     ...
     * 添加店铺信息
     */
    public function addDineshop($shopname, $shopdesc, $shopicon, $cuisineid, $maplon, $maplat, $sales, $deliveryfee, $minprice, $preconsume, $isbooking, $isaway, $opentime, $shophone, $address, $adduser, $minconsume,$servicecharge,$isrecom){
        try{
            $data = array(
                'f_adduser' => $adduser,
                'f_shopname' => $shopname,
            ...
            if($isrecom == 1){
                ...
                Db::table("t_dineshop_recom")->insert($data1);
            }
     ...
     * 修改店铺信息
     */
    public function modDineshop($shopid, $shopname, $shopdesc, $shopicon, $cuisineid, $maplon, $maplat, $sales, $deliveryfee, $minprice, $preconsume, $isbooking, $isaway, $opentime, $shophone, $address, $minconsume,$servicecharge,$tuijian){
        // 启动事务
        Db::startTrans();
        try{
            $data = array(
     ...
     * 修改店铺桌型
     */
    public function editDesk($id, $deskid, $seatnum ,$desknum )
    {
    ...
    public function getDesklist($shopid){
        $desklist = Db::table('t_dineshop_deskinfo')
            ->field('f_id id,f_deskid deskid, f_sid shopid, f_seatnum seatnum, f_amount desknum, f_orderamount ordernum,f_status status, f_addtime addtime')
            ->where('f_sid', $shopid)
            ->order('f_deskid asc')
            ->select();
        return $desklist;
