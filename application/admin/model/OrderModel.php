...
class OrderModel extends Model
{
    ...
    {   
        ...
        $where = "f_type = 2";
        $orders = Db::table('t_orders')->where($where)->order('f_addtime desc')->select();
        // error_lo('new1',Db::getlastsql());
     ...
     *  搜索订单
     **/
    public function findOne($name,$dataDay,$dataDay1,$timeDay,$timeDay1)
    {   
        $status = array(
            '0'=>'初始',
            '1'=>'未付款',
     ...
     *  模糊查询菜肴统计
     */
    public function getSearchAdvance($dineshop='',$foodname='')
    {
      $where = 'a.f_state = 1';
      if(!empty($foodname)){
     ...
     *  时间查询菜肴统计
     */
    public function getSearchTimer($dataDay='',$dataDay1='',$timeDay='',$timeDay1='',$foodname='',$dineshop='')
    {
      $where = 'a.f_state = 1';
      if(!empty($timeDay) && !empty($timeDay1)){
