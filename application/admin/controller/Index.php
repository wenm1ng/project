...
class Index extends Base_1
{
    ...
    public function home(){
        ...
        //30天内访问人次
        $person_time = Db::name('count')->whereTime('create_time','between',["{$first_time}","{$second_time}"])->count();
        //1年内访问人次
        $person_time_year = Db::name('count')->whereTime('create_time','between',["{$year_time}","{$second_time}"])->count();
        //历史访问人数
        $person_num = Db::name('count_person')->count();
