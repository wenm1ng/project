...
$(function(){
    ...
    window.updateAlert = function (text,c) {
		...
		layer.alert(text, {icon: icon, title : "提醒信息"});
        return;
        text = text||'default';
		c = c||false;
		if ( text!='default' ) {
...
function highlight_subnav(url){
    ...
    var fpid = $subnav.find("a[href='" + url + "']").attr('pid');

    $subnav.find("a[href='" + url + "']").parent().addClass("active");
    $subnav.find("a[href='" + url + "']").parent().parent().parent().addClass("active opened active expanded ");

    $("li[fpid="+fpid+"]").addClass('active opened active expanded ');
    $subnav.find("a[href='" + url + "']").css('background-color', '#3f4142');
